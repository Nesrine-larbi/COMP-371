...
int compileAndLinkSkyboxShaders()
{
    ...
    {
        glGetShaderInfoLog(vertexShader, 512, NULL, infoLog);
        std::cerr << "ERROR::SKYBOX::VERTEX::COMPILATION_FAILED\n" << infoLog << std::endl;
    }
    ...
    {
        glGetShaderInfoLog(fragmentShader, 512, NULL, infoLog);
        std::cerr << "ERROR::SKYBOX::FRAGMENT::COMPILATION_FAILED\n" << infoLog << std::endl;
    }
    ...
    if (!success) {
        glGetProgramInfoLog(shaderProgram, 512, NULL, infoLog);
        std::cerr << "ERROR::SKYBOX::PROGRAM::LINKING_FAILED\n" << infoLog << std::endl;
    }
...
unsigned int loadCubemap()
{
    ...
    unsigned char *data = stbi_load("assets/skybox/space3.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
        // Load the same image for all 6 faces of the cubemap
...
int main(int argc, char*argv[])
{
    ...
    int skyboxShaderProgram = compileAndLinkSkyboxShaders();
    ...
    // Setup skybox
    unsigned int skyboxVAO = createSkyboxVBO();
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // Draw skybox first (early Z-test optimization)
        glDepthFunc(GL_LEQUAL);  // change depth function so depth test passes when values are equal to depth buffer's content
        glUseProgram(skyboxShaderProgram);
        mat4 skyboxView = mat4(mat3(viewMatrix)); // remove translation from the view matrix
        ...
        // Switch back to main shader for regular geometry
        glUseProgram(shaderProgram);
        ...
        // Reset view matrix after drawing the spinning cube in view space
        if (cameraFirstPerson) {
